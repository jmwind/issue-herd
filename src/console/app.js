  ...
  function pickerSheet() {
    var opts = factories().map(function (f) {
      ...
      return '<button class="opt' + (f.id === chosen ? ' on' : '') + (alive ? '' : ' stale') + '" data-choose="' + esc(f.id) + '"><i class="led ' + (alive ? 'green' : 'red') + ' still"></i><span class="n">' + esc(f.name) + '</span>' +
        '<span class="c"><span class="pill">' + f.counts.running + ' assembling</span>' + (f.counts.alerts ? '<span class="pill hot">' + f.counts.alerts + ' alert' + (f.counts.alerts > 1 ? 's' : '') + '</span>' : '') + '</span>' +
    ...
    if (factories().length > 1) opts = '<button class="opt' + (chosen === 'all' || !current() ? ' on' : '') + '" data-choose="all"><i class="led green still"></i><span class="n">All factories</span><span class="c"><span class="pill">' + factories().reduce(function (s, f) { return s + f.counts.running; }, 0) + ' assembling</span></span><span class="m">every factory on ' + esc(document.body.dataset.hostname) + '</span></button>' + opts;
    var f = current(), team = '';
