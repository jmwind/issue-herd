...
// team beside it, the versioned routes, the event stream with cursors, commands as tracked
...
import { teamPaths, writeRegistration, SqliteStore, storePath } from '@weawr/engine';
...
test('serve: an online owner and an offline team, every fact from the owner or the store, validated against the protocol', async (t) => {
  ...
  const snap = await until(async () => { const s = await fetch(`${h.url}/api/v1/snapshot`).then(j); return s.result.teams.length === 2 ? s.result : null; });
  const v = validate(hostSnapshotSchema, snap);
  assert.ok(v.ok, v.ok ? '' : describeIssues(v.issues));
  const byId = Object.fromEntries(snap.teams.map((f) => [f.teamId, f]));
  ...
  const one = await fetch(`${h.url}/api/v1/teams/fon/snapshot`).then(j);
  assert.ok(validate(teamSnapshotSchema, one.result).ok);
  const list = await fetch(`${h.url}/api/v1/teams`).then(j);
  assert.deepEqual(list.result.teams.map((f) => [f.teamId, f.owner.status]).sort(), [['foff', 'offline'], ['fon', 'online']]);
  const task = await fetch(`${h.url}/api/v1/teams/fon/tasks/GH-1`).then(j);
  ...
  assert.equal(legacy.teams.length, 2); assert.equal(legacy.hostname, os.hostname());
});

test('serve: commands are tracked operations answered by the owner; a repeat is one action; an offline team refuses mutations; a lost host stops nothing', async (t) => {
  ...
  await until(async () => (await fetch(`${h.url}/api/v1/snapshot`).then(j)).result.teams.length === 2);
  ...
  const bad = await post('task.done', { team: 'fact' }).then(j);
  assert.equal(bad.ok, false); assert.equal(bad.error.code, 'bad_request'); assert.match(bad.error.message, /task: must be a string/);
  const done = await post('task.done', { team: 'fact', task: 'GH-1', requestId: 'r1' }).then(j);
  ...
  const again = await post('task.done', { team: 'fact', task: 'GH-1', requestId: 'r1' }).then(j);
  ...
  const op = await fetch(`${h.url}/api/v1/teams/fact/operations/${done.result.operation.id}`).then(j);
  assert.equal(op.result.status, 'completed'); assert.equal(op.result.kind, 'task.done');
  const snap = await until(async () => { const s = (await fetch(`${h.url}/api/v1/teams/fact/snapshot`).then(j)).result; return s.issues[0].cleared ? s : null; });
  ...
  const refused = await post('task.done', { team: 'faway', task: 'GH-2', requestId: 'r2' }).then(j);
  ...
  const back = await until(async () => { const s = (await fetch(`${h2.url}/api/v1/snapshot`).then(j)).result; return s.teams.find((f) => f.teamId === 'fact')?.owner.status === 'online' ? s : null; });
  assert.equal(back.teams[0].owner.status, 'online');
});
...
test('serve: the event stream sends a snapshot, then the owner\'s events with cursors; a resume replays only what is new', async (t) => {
  ...
  await until(async () => (await fetch(`${h.url}/api/v1/snapshot`).then(j)).result.teams.length === 1);
  const client = new WeawrClient({ baseUrl: h.url, token });
  ...
  const rev = got.snapshots[0].teams[0].revision;
  assert.ok(rev >= 1);
  ...
  assert.ok(got.events.every((e) => e.teamId === 'fev'));
  sub.close();
  // a fresh subscription from the cursor gets the events after it, not everything again
  const last = seqs[seqs.length - 1];
  const res = await fetch(`${h.url}/api/v1/teams/fev/events?after=${last - 1}`).then(j);
...
test('serve: a gated host locks the page, refuses the API without a session, and lets a device in by token', async (t) => {
  ...
  const snap = await until(async () => { try { const s = await dev.hostSnapshot(); return s.teams.length ? s : null; } catch { return null; } });
  assert.equal(snap.teams[0].teamId, 'fg');
