...
/**
 * The web console's build: app.html, app.css, app.js, unlock.html and assets/{icons,logo}. The
 * assembled CLI keeps it in web/ next to itself; a test passes the page's own source directory.
 */
export const DEFAULT_WEB_DIR = path.join(HERE, 'web');
function asset(webDir, name) { return fs.readFileSync(path.join(webDir, name), 'utf8'); }
...
function mark(webDir) {
  try {
    const svg = fs.readFileSync(path.join(webDir, 'assets', 'logo', 'weawr-mark-reverse.svg'), 'utf8').replace('<svg ', '<svg class="mark" ').replace(' role="img" aria-label="weawr"', ' aria-hidden="true"');
...
function icon(webDir, file) { try { return fs.readFileSync(path.join(webDir, 'assets', 'icons', file)); } catch { return null; } }
 ...
 */
export function createHandler({ gate, console: app, hostname = os.hostname(), log = () => {}, webDir = DEFAULT_WEB_DIR }) {
  const clients = new Set();
  const pages = { app: asset(webDir, 'app.html'), unlock: asset(webDir, 'unlock.html'), css: asset(webDir, 'app.css'), js: asset(webDir, 'app.js') };
  const logo = mark(webDir);
  const icons = Object.fromEntries(Object.entries(ICONS).map(([p, [file, type]]) => [p, { body: icon(webDir, file), type }]));
