// `weawr migrate [--dry-run]`: from state.json to the durable store, under the factory's lock.
...
export async function migrate(ctx: Context, args: string[]): Promise<void> {
  ...
  console.log(`factory ${cfg.name} (${ctx.paths.repo})`);
  ...
  console.log(`  console acknowledgements for this factory: ${inv.consoleNotes}`);
  ...
  try {
    ...
    console.log(`rollback: stop weawr, move ${rel(ctx.paths.statePath)}.migrated back to state.json and remove factory.sqlite* — only before new work has started; afterwards the store is the truth`);
