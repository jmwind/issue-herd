...
export async function serve(ctx: Context, args: string[], { asConsole = false } = {}): Promise<void> {
  ...
  if (args[0] && !args[0].startsWith('--')) throw new Error(`usage: weawr ${asConsole ? 'console' : 'serve'} [--port N] [--host ADDR]... [--no-web] | set-passcode | clear-passcode | device add|list|revoke`);
  ...
  const { handler } = createHandler({ gate, hub, log, webDir: args.includes('--no-web') ? null : ctx.webDir, devices: () => creds().devices || {}, version: ctx.version });
  let bound;
