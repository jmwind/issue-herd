...
import { ask, credentialsPath, deleteCredential, noCredentialError, resolveCredential, saveCredential, terminalUi } from '@weawr/engine/adapters/auth.mjs';
import { Herdr, agentNameFor, agentPlacement, isBlocked, isNameTaken } from '@weawr/engine/adapters/herdr.mjs';
import { newerVersion } from '@weawr/engine/version.mjs';
import { desiredBranch, reconcileBranch } from '@weawr/engine/adapters/branch.mjs';
import { catchUp, defaultBranch, makeWorktree, pullBase, removeWorktree } from '@weawr/engine/adapters/worktree.mjs';
import { agentArgv, describeAgent, exitCommandFor, TRANSLATED_KINDS } from '@weawr/engine/agents.mjs';
import { conflictPrompt, keepMergeable, parsePrUrl, prState, watchesMerge } from '@weawr/engine/adapters/pr.mjs';
import { DEFAULT_MAX_NUDGES, normalizeMaxNudges, nudgeInstructions, nudgeQuote, nudgedByLabel, nudgesIn, nudgesLeft, nudgesSent, planNudge } from '@weawr/engine/nudge.mjs';
import { GitHubTracker } from '@weawr/engine/adapters/trackers/github.mjs';
import { askSecret, loadCredentials } from '@weawr/engine/adapters/auth.mjs';
import { stampFactory } from './console/registry.mjs';
import { Gate, hashPasscode } from './console/passcode.mjs';
import { FactoryConsole } from './console/console.mjs';
import { createHandler, listen, tailscaleAddresses } from './console/server.mjs';
...
const PKG = { version: typeof __WEAWR_VERSION__ !== 'undefined' ? __WEAWR_VERSION__ : readJson(path.join(PKG_DIR, '..', 'package.json'), { version: '0.0.0' }).version };
const HELP = `weawr — watch an issue tracker (Linear or GitHub Issues); when an issue matches a rule, open
a git worktree and a herdr workspace, start a coding agent in it, brief it, and report back.
...
async function consoleCommand(args) {
  ...
  const { handler, broadcast } = createHandler({ gate, console: app, log: (m) => log(m), webDir: path.join(PKG_DIR, 'web') });
