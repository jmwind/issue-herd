...
// usage text. Nothing here knows about teams.
...
a git worktree and a herdr workspace, start a coding agent in it, brief it, and report back.
  ...
  weawr serve [--port N] [--host ADDR] [--no-web]  the CLI interface (/api/v1, events) for every team here, with the web console
  weawr console [--port N] [--host ADDR] [--theme factorio|clean|linear|github|tokyo-night|solarized-light]  the same, by its older name: the team room in a browser (phone first)
  ...
  weawr migrate [--dry-run] move a team's state.json into the durable store (the watcher does this on start)
  ...
  weawr plugins [examples]  which plugins this team enables (intake, roles, scheduled tasks), and what ships
  weawr demo [list|<scenario>|reset]  a team to try weawr on, against the demo repository: file the scenario's issues, run, reset
