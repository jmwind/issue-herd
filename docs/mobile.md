...
`weawr serve` on the machine that runs the teams (Tailscale reaches it; a passcode gates it).
...
2. `GET /api/v1/snapshot` — every team; or `/api/v1/teams/<id>/snapshot` for one.
3. `GET /api/v1/events?cursors=<teamId>:<seq>,…` — keep one cursor per team (each
   team snapshot's `revision` is the first), resume after it on reconnect, refetch a team
   when the stream says `resnapshot`.

Show `owner.status` (`online` / `stale` / `offline`) and `freshness` on every team, and your
