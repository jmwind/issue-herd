...
packages/
  engine/     the team: lifecycle, commands, observations; adapters for trackers, herdr, git, persistence
...
One watcher process per repository (`weawr` in a herdr pane) hosts one `TeamEngine`. Before it
schedules, mutates or reconciles anything it takes **exclusive local ownership** of the team:
...
Each owner **registers itself** in its own file, `~/.config/weawr/teams/<teamId>.json`,
replaced atomically every poll — no shared read-modify-write registry. The record carries where the
team is, where its owner answers, and, separately, when the tracker last answered
...
A team's state is an SQLite database, `<repo>/.weawr/state/team.sqlite` (`node:sqlite`, no
...
(`TeamEngine.commit`); the external work is performed after the commit and its outcome
...
observable is blindly repeated. `packages/engine/src/store/sqlite.ts`, `team.ts`.
...
refused and left untouched — never read as an empty team — and a reader shows that state too.
There is no dual write: after migration the JSON file is a backup. Rollback (move it back, remove
`team.sqlite*`) is only sound before new work has started; afterwards the store is the truth.
`packages/engine/src/store/migrate.ts`.

## Recipes, results and the merge

The effective prompt setup of a team is a **recipe revision**: a set of templates and the
result/nudge protocol they teach. Revisions are bundled side by side
(`packages/recipes/prompts/<revision>/`) and kept: a team is pinned to one (`recipe_revision`
in its store — revision 1 for a migrated team, the latest for a new one), and a task keeps the
...
is not touched by an upgrade. `packages/recipes/src/manifest.ts`, `TeamEngine.upgradeRecipe`.
...
decisions (`TeamEngine.ruleFor`): editing its rule's cleanup, agent or merge settings does not
...
The **merge** (`weawr merge <run key>`, `TeamEngine.mergeRun`) is the one route to an unattended
...
forwards commands to each team's owner over the private socket and gathers their snapshots; it
...
| `POST /api/v1/commands/<name>` | `task.done`, `task.undo`, `task.stop`, `task.reset`, `task.tail`, `run.exit`, `run.tail`, `team.tidy` — bodies checked against `packages/protocol/src/envelope.ts`; every mutation carries a `requestId` |
...
refused. Request ids are scoped to the team and the caller. Every mutation against a team
...
shell) that keeps a cursor per team, resumes after it on reconnect, refetches a team when
...
| team | host + repository path |
| task | team + the tracker's own scope (`github:owner/repo`, `linear:TEAM`) + issue key |
| role run | task + role — what state has always called the run key |
| attempt | role run + pass |

herdr agent names are the run key plus the team's short hash (`gh-7-impl-a1b2c3`), so two
...
subscribes to `/api/v1/events` with a cursor per team; it shows `owner.status`,
...
| The lifecycle (pickup, supervise, finalize, merges, nudges) | `packages/engine/src/team.ts` |
